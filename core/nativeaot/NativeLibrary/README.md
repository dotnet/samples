...
When linking the generated static library, it is important to also include additional framework dependencies in the linker settings, and add `NativeAOT_StaticInitialization` to the symbol table. In MSVC, this can be accomplished by appending the following flag to the linker settings:
```
/INCLUDE:NativeAOT_StaticInitialization
```

You can find a list of additional framework libraries by publishing the project with detailed verbosity (`-v d`), and looking at the output generated by the `LinkNative` target.
