...
When linking the generated static library, it is important to also include additional framework dependencies in the linker settings, and add `NativeAOT_StaticInitialization` to the symbol table. On Windows, this can be accomplished by appending the following flag to the linker settings:
```
