...
#
# This script builds the [helloworld] GOLANG executables and writes
...
$env:GOPATH   =  $goRoot
$projectPath  = "$env:GOPATH\src\shared-c\cmd\helloworld"
...
$logPath = "$logPath\build-helloworld.log"
...
$env:GOARCH      = "amd64"
$env:CGO_ENABLED = 1
go build -i -v -buildmode=c-shared -o $buildPath\helloworld.win.dll "$projectPath\main.go" > "$logPath" 2>&1

$exitCode = $lastExitCode

if ($exitCode -ne 0)
{
    Write-Error "*** ERROR: [helloworld] WINDOWS build failed.  Check build logs: $logPath"
