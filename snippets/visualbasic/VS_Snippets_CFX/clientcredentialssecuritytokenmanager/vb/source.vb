...
Namespace Microsoft.ServiceModel.Samples
	...
	Public Class Client
		Shared Sub Main(ByVal args() As String)
			...
			Dim securityBE As AsymmetricSecurityBindingElement = SecurityBindingElement.CreateMutualCertificateDuplexBindingElement(MessageSecurityVersion. WSSecurity10WSTrustFebruary2005WSSecureConversationFebruary2005WSSecurityPolicy11BasicSecurityProfile10)
			...
			Using factory As New ChannelFactory(Of IMyServiceChannel)(binding, serviceEndpoint)
				...
				factory.Endpoint.Behaviors.Remove(GetType(ClientCredentials))
				factory.Endpoint.Behaviors.Add(credentials)

				Dim channel As IMyServiceChannel = factory.CreateChannel()
	...
	Friend Class Service
		...
		Shared Sub Main(ByVal args() As String)
			...
			Using host As New ServiceHost(GetType(Service), serviceEndpoint)
				Dim binding As New CustomBinding()
				binding.Elements.Add(SecurityBindingElement. CreateMutualCertificateDuplexBindingElement(MessageSecurityVersion.WSSecurity10WSTrustFebruary2005WSSecureConversationFebruary2005WSSecurityPolicy11BasicSecurityProfile10))
				...
				SetupCertificates(credentials)
				host.Description.Behaviors.Remove(GetType(ServiceCredentials))
				host.Description.Behaviors.Add(credentials)

				Dim endpoint As ServiceEndpoint = host.AddServiceEndpoint(GetType(IMyService), binding, "")
				host.Open()
