...
public:
    ...
    )
    {
        ...
        if (NULL != pwszPhysicalPath)
        {
            ...
            {
                // Convert the physical path string.
                pszPhysicalPath = (char*) pHttpContext->AllocateRequestMemory( (DWORD) wcslen(pwszPhysicalPath)+1 );
                wcstombs(pszPhysicalPath,pwszPhysicalPath,wcslen(pwszPhysicalPath));
