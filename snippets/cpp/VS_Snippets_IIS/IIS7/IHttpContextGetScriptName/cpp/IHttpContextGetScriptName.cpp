...
public:
    ...
    )
    {
        ...
        pwszScriptName = pHttpContext->GetScriptName(&cbScriptName);
        
        // Test for an error.
        if ((pwszScriptName != NULL) && (cbScriptName > 0))
        {
            // Create a non-unicode buffer for the script information.
            pszScriptName = (char*) pHttpContext->AllocateRequestMemory(
                (DWORD) wcslen(pwszScriptName)+1 );
            // Test for an error.
            if (pszScriptName != NULL)
            {
                // Send the script information to the client.
                wcstombs(pszScriptName,pwszScriptName,wcslen(pwszScriptName));
                WriteResponseMessage(pHttpContext,
                    "Script Name: ",pszScriptName);
