...
protected:
    ...
    // The Write method writes IFileKey
    // pointer information to the event log.
    // fileKey: the IFileKey 
    // pointer to write.
    // throws: a CEventException exception.
    void Write
    (
    ...
    {
        // If the IFileKey pointer is NULL,
        // throw a CEventException.
        if (NULL == fileKey)
        {
            CEventException ce
                (L"NULL IFileKey pointer");
            throw ce;
        }

        // Get the cache name from 
        // the IFileKey pointer.
        wstring cacheName = 
            fileKey->GetCacheName();

        // Write the cache name information
        // to the event log.
        ReportInfo(L"IFileKey::GetCacheName",
                   cacheName);
    }
