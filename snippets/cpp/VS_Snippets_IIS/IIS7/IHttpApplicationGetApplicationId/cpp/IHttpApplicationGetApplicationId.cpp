...
public:
    ...
    )
    {
        ...
        PCWSTR pwszApplicationId =
            pHttpApplication->GetApplicationId();

        // Test for an error.
        if (NULL != pwszApplicationId)
        {
            // Length of the returned path.
            DWORD cbApplicationId =
                (DWORD) wcslen(pwszApplicationId);
            // Allocate space to store the path.
            pszApplicationId =
                (PSTR) pHttpContext->AllocateRequestMemory(cbApplicationId + 1);
            // Test for an error.
            if (pszApplicationId==NULL)
            {
            ...
            wcstombs(pszApplicationId,pwszApplicationId,cbApplicationId);
            ...
            WriteResponseMessage(pHttpContext,
                "Application ID: ",pszApplicationId);
